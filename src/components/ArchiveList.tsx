...
// Keep document-scrolled rendering deliberately small until the list is
// virtualized. A smaller increment limits both cards and their controls.
...
export function ArchiveList({ data, query, setQuery, filter, setFilter, sort, setSort, counts, visible, patchItem, recoveryBusy, captureContent, onNotice, shareNotice, showPostPanels, postsByItem }: {
  ...
  const [renderLimit, setRenderLimit] = useState(RESULTS_PER_PAGE);
  const [postPanelItemId, setPostPanelItemId] = useState<string | null>(null);
  ...
  return <section className="archive-main" aria-label="Saved pieces">
    ...
    {visible.length ? <><ol className="archive-list">{renderedItems.map((item) => <ArchiveListItem key={item.id} item={item} itemPosition={itemPositions.get(item.id) ?? 0} collections={data.collections} patchItem={patchItem} recoveryBusy={recoveryBusy} captureContent={captureContent} onNotice={onNotice} showPostPanels={showPostPanels} postPanelItemId={postPanelItemId} setPostPanelItemId={setPostPanelItemId} post={postsByItem.get(item.id) ?? null} />)}</ol>{renderedItems.length < visible.length ? <button type="button" className="archive-show-more" onClick={() => setRenderLimit((limit) => limit + RESULTS_PER_PAGE)}>Show {Math.min(RESULTS_PER_PAGE, visible.length - renderedItems.length)} more pieces</button> : null}</> : <div className="archive-empty"><span aria-hidden="true">◇</span><p>No pieces found.</p><small>Try a broader search, another filter, or save something from the reader.</small>{query || filter !== "all" ? <button type="button" onClick={() => { setQuery(""); setFilter("all"); }}>Clear search and filters</button> : null}</div>}
  </section>;
}

function ArchiveListItem({ item, itemPosition, collections, patchItem, recoveryBusy, captureContent, onNotice, showPostPanels, postPanelItemId, setPostPanelItemId, post }: {
  item: ArchiveItem;
  itemPosition: number;
